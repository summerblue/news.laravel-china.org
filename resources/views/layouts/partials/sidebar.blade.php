
    <div class="widget rm-padding grid-item">
        <div class="grid-wrap subscribe-wrap">
            @include('layouts.partials.weekly_card', ['size' => 'small'])
        </div>
    </div>

    <div class="widget grid-item links">
        <div class="grid-wrap">
            <h4 class="title">推荐链接</h4>
            <a href="https://laravel-china.org/" class="btn btn-default btn-block" target="_blank">Laravel China 社区</a>
            <a href="https://doc.laravel-china.org/docs" class="btn btn-default btn-block" target="_blank">Laravel 中文文档</a>
            <a href="https://cs.phphub.org/" class="btn btn-default btn-block" target="_blank">Laravel 速查表</a>
            <a href="http://laravel.so/" class="btn btn-default btn-block" target="_blank">Laravel 开发技巧</a>
            <a href="https://lumen.laravel-china.org/" class="btn btn-default btn-block" target="_blank">Lumen 中文文档</a>
            <a href="https://laravel-china.org/laravel-tutorial/5.1/about" class="btn btn-default btn-block" target="_blank"><i class="fa fa-paper-plane" aria-hidden="true"></i> Laravel 入门教程</a>
        </div>
    </div>


    <div class="widget grid-item">
        <div class="grid-wrap">
            <h4 class="title">关注订阅号</h4>
            <div class="content">
                <img src="/assets/images/qrcode_squre.jpg" alt="" style="width:100%"/>
            </div>
        </div>
    </div>
